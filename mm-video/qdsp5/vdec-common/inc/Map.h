...
public:
    ...
    void insert(T,T2 d2=0);
    void show();
    int  size();
    T2 find(T); // Return VALUE
    T find_ele(T);// Check if the KEY is present or not
    T2 begin(); //pop the "VALUE" of the first node
    T get_front_key();// pop the KEY of the first node
    bool erase(T);
    bool eraseall();
    bool isempty();
    ~Map()
...
T2 Map<T,T2>::find(T d1)
{
    ...
    while(tmp)
    {
        if(tmp->data == d1)
        {
            // return the value for the found key
            return tmp->data2;
        } 
        tmp = tmp->next;
    }
