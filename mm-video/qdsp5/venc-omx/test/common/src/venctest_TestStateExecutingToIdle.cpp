...
$Header: //source/qcom/qct/multimedia2/Video/Enc/VencOmx/rel/1.3/test/common/src/venctest_TestStateExecutingToIdle.cpp#1 $
...
namespace venctest
{
                                                   ...
                                                   OMX_S32 nTestNum)
   {
      ...
      for (OMX_S32 i = 0; i < nRuns; i++)
      {
         ...
         VENC_TEST_MSG_HIGH("Enter idle %d", (int) i, 0, 0);
         result = CheckError(m_pEncoder->SetState(OMX_StateIdle, OMX_TRUE));
         if (result != OMX_ErrorNone)
         {
            VENC_TEST_MSG_ERROR("failed enter idle state %d", (int) i, 0, 0);
            break;
         }

         VENC_TEST_MSG_HIGH("Check queues %d", (int) i, 0, 0);
